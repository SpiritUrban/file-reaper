...
/// Стан сурогата вже карантинованого запису (T-156).
///
/// Потрібен звірці при старті: purge знищує файл і лише потім позначає запис
/// `Purged` — аварія між цими кроками лишає запис, який обіцяє відновлення
/// неіснуючого файла. Відрізнити «файла справді немає» від «том зараз
/// недоступний» може лише FS, тому це питання до шлюзу, а не до use case.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SurrogateState {
    /// Файл на місці — запис відновлюваний.
    Present,
    /// Каталог карантину тому доступний, а файла в ньому немає: він знищений.
    Missing,
    /// Каталог карантину тому недоступний (том від'єднано, ще не готовий) —
    /// про запис не відомо нічого; чіпати його не можна.
    RootUnavailable,
}
...
pub trait QuarantineFs {
    ...
    /// Read-only стан сурогата карантинованого запису при старті (T-156).
