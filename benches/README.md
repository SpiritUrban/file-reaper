...
Наповнюються задачами T-019, T-035, T-066, T-076, T-154, T-157.
...
| RAM Core після скану 1 ТБ | < 300 МБ | `mem-bench` (**справжній RSS процесу**, T-157) + `scan-bench`/`index-bench` (оцінка heap) | RSS-стеля 300 МБ жорстко; оцінка звіряється з реальним RSS |
...
## `mem-bench` — гейт пам'яті Core (T-157)

Ціль §15: **RAM Core у простої після скану < 300 МБ.** На відміну від
index/scan-bench, які гейтять *оцінку* heap (`capacity()`), тут міряється
**справжній working set (RSS)** процесу через WinAPI (`GetProcessMemoryInfo`) —
джерело правди для §15.

```sh
cargo run --release --manifest-path benches/mem-bench/Cargo.toml   # CI check (Windows)
```

| Інваріант | Гейт | Пояснення |
|---|---|---|
| `idle_rss` | **стеля §15 300 МБ жорстко** | RSS процесу після 1.5 млн файлів + `finish_indexing` |
| `estimate_honesty` | **жорстко** | справжній RSS не більший за оцінку `memory_usage()` × 1.6 — інакше оцінка бреше і гейти index/scan-bench перестають захищати |
| `for_each_mut peak delta` | **жорстко (< 64 МБ)** | повний прохід ферми/перерахунку через `HotIndex::for_each_mut` не матеріалізує індекс; регрес назад до `get_all()` (спайк +190 МБ, до 418 МБ) валить гейт |

Заміри на еталонному наборі: RSS у простої ≈ 109 МБ (оцінка ≈ 104 МБ),
приріст RSS повного проходу категоризації ≈ 0 МБ. Стелі абсолютні
(RSS машинозалежний), але сама стеля §15 — детермінований інваріант.

Windows-only (RSS через WinAPI); на інших ОС — no-op (exit 0).
