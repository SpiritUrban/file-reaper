...
### ⚠️ Сайт відстає від релізу, хоча деплой зелений

Спостережено на `v0.1.0`: деплой з тега — найновіший і `success`, крок перевірки маніфесту
зелений, а живий URL віддає попередню збірку (`Last-Modified` вказує на деплой з `main`).
Причина невідома; це відомий незакритий баг, описаний у розділі 11 брифу Стадії 2.

**Обхід:** `Actions → Deploy GitHub Pages → Run workflow` з гілки `main`. Поза тегом маніфест
запитує `releases/latest`, який уже віддає опублікований реліз, — вітрина наздоганяє без
повторного тегування.

**Що це НЕ ламає:** інсталятори, `latest.json` і автооновлення живуть у GitHub Releases, а не на
Pages. Застарілий сайт — неправильна вітрина, а не зламаний продукт.
