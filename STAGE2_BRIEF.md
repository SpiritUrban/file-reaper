...
### Ревізія 4: відтворено втретє — і перевірка з ревізії 3 це пропустила

`SpiritUrban/file-reaper`, тег `v0.1.0`. Симптом той самий, докази ще чіткіші:

| Джерело | Що казало |
|---|---|
| Deployments API | деплой з тега — найновіший (`ref: v0.1.0`), `success` |
| Крок `Verify manifest matches the tag` | `success` — на момент деплою маніфест мав 8 ассетів |
| Живий URL із кеш-бастером | `source: fallback`, **0 ассетів** |
| `Last-Modified` живого файла | вказує на **попередній** деплой з `main`, на 8 годин раніше |
| `Age` | 84 с — тобто це свіжа відповідь від origin, а не кеш CDN |

**Головний урок не про Pages, а про перевірку.** Крок «Verify the live site» з ревізії 3
порівнював лише `version` — і був зелений, бо fallback-маніфест бере версію з `package.json`, тобто
**ту саму** `0.1.0`. Перевірка не була здатна почервоніти на випадку, заради якого існує.

Порівнювати треба **підпис збірки**, а не одне поле:

```bash
signature() {
  node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{
    try{const j=JSON.parse(s);console.log([j.version,j.source,j.tag,(j.assets||[]).length].join('/'))}
    catch{console.log('')}})"
}
# fallback → 0.1.0/fallback//0     з тега → 0.1.0/tag/v0.1.0/8
```

Перевірити це можна за 10 секунд локально, на двох файлах-зразках, — і саме так треба робити з
кожною перевіркою, перш ніж на неї покладатись.

**Обхід, який спрацював:** ручний `Actions → Deploy GitHub Pages → Run workflow` з `main`. Поза
тегом маніфест запитує `releases/latest`, а він уже віддає опублікований реліз з ассетами — тобто
вітрина наздоганяє без повторного тегування.
